...
# Traduz a mensagem para o Português (Para o login do Gato)
login_manager.login_message = "Por favor, faça login para acessar esta página."
login_manager.login_message_category = "warning"

# --- A FUNÇÃO QUE ESTAVA FALTANDO (CORREÇÃO DO ERRO) ---
...
# --- GUARDA DE SEGURANÇA (Obriga a trocar a senha) ---
@app.before_request
def check_password_change():
    # O erro acontecia aqui porque o load_user não existia
...
class User(UserMixin, db.Model):
    ...
    resposta_seguranca = db.Column(db.String(255), nullable=False)
...
def migrar_bruna():
    ...
    db.session.commit()
    return "Sucesso! Usuária migrada. Login: bruna.emanuele / Senha Temp: Mudar@123"
...
def register():
    if request.method == 'POST':
        ...
        hashed_resp = generate_password_hash(resposta.lower().strip(), method='scrypt')
